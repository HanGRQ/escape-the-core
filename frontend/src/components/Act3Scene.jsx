...
// Note: default persona is 'caring' — the Caring Tone is unlocked after Act II
// and is the persona active during Act III, per GDD §5.6. ('ally' unlocks
// only after Act III completes, and is used during the Final Quiz.)
export function Act3Scene({ sessionId, userId, personaStage = 'caring', onComplete }) {
  ...
  // 'task'     — Prompt Lab replaces the lecture in the RIGHT panel
  ...
  const sendChat = useCallback((message) => {
    ...
    api.streamChat('room_3', { sessionId: sid, userId, message, history }, {
      ...
      onError: () => {
        setFeed(prev => { const u = [...prev]; if (u[u.length - 1]?.streaming) u[u.length - 1] = { ...u[u.length - 1], content: 'Error.', streaming: false }; return u })
        setChatLoading(false)
      },
    })
  }, [sid, userId, feed])
  ...
  const handleComplete = useCallback(async () => {
    try { await api.completeRoom('room_3', { sessionId: sid, userId, score: 1.0 }) } catch {}
    // Act III completion unlocks the Ally Tone, used during the Final Quiz.
    onComplete('ally')
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden flex">
      ...
      <div className="relative z-10 flex-shrink-0 h-full flex flex-col"
        style={{ width: '34%', borderRight: `1px solid ${ACCENT}33`, background: 'rgba(3,8,13,0.6)' }}>
      ...
      {/* ── Right — Teaching, then Prompt Lab ── */}
      <div className="relative z-10 flex-1 min-w-0 flex flex-col">
        <div className="flex items-center justify-between px-6 py-2 flex-shrink-0"
          style={{ borderBottom: `1px solid ${ACCENT}33`, background: 'rgba(3,8,13,0.5)' }}>
        ...
        <div className="flex-1 min-h-0 p-6 overflow-hidden" style={{ background: 'rgba(3,8,13,0.3)' }}>
          {phase === 'teaching' ? (
            <TeachingPanel
              accent={ACCENT}
        ...
        <div className="flex items-center justify-end px-6 py-1.5 flex-shrink-0"
          style={{ borderTop: `1px solid ${ACCENT}22`, background: 'rgba(3,8,13,0.55)' }}>
